...
Nix Seed pre-builds dependencies and stores them in a *registry* - a software
warehouse.
...
If an attacker tampers with a pre-built package, every program built from it is
compromised - and nobody knows. This is a *supply chain attack*. In 2020,
...
- An *SBOM* (Software Bill of Materials) - a complete ingredient list of every
  library and tool that went into the build.
...
*Full-Source Bootstrap* means building the entire compiler chain from scratch,
starting from a tiny program small enough for a human to read and verify by
hand. No pre-compiled binary is trusted. Every tool in the chain is built from
an auditable source.
...
transparency log operated by the Sigstore project. A transparency log is
append-only and publicly readable. Entries can be added but not removed or
modified. Changing past entries is detectable.
...
- **Rekor limitations:** Sigstore is a US-operated public-good service with no
  uptime guarantee. If Rekor is down, attestations cannot be verified or
  recorded and builds are blocked. The log's append-only property prevents
  silent deletion of past records, but does not prevent a compromised or
  compelled operator from inserting false records going forward.
- **Cost:** Free.

#### Credulous

Multiple independent servers across different organisations and countries each
build the package and record the fingerprint on *Rekor*. A *Master Builder* - a
designated coordinator server - signs off only when a *quorum* - a minimum
number of independent builders - agree on the result.
...
A *blockchain* is a public ledger maintained by thousands of servers. No single
server owns it. Entries are permanent: changing them would require agreement
from more than half the network, which is a practical impossibility.
...
- **Legal compulsion:** 2-of-3 can potentially be satisfied by one powerful
  ...
  four genuinely independent jurisdictions simultaneously.
...
The organisations that run the internet's core services (domain name resolution,
...
referenced above was a Russian state operation: attackers compromised the
SolarWinds Orion build pipeline and signed the backdoor with the legitimate key.
It passed every check because there was only one build to compromise.

A quorum confined to any single jurisdiction - or to jurisdictions bound by
intelligence-sharing agreements - is a single target.
...
[Zero](#zero) enforces this in the smart contract: builder metadata, including
controlling organisation, jurisdiction, geographic location, and tech stack, is
encoded in the contract and used to verify independence before counting any
attestation toward quorum. No single jurisdiction can satisfy the quorum
requirement alone, regardless of how many builders it controls.
